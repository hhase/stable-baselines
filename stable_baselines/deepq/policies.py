        ...
        with tf.variable_scope("model", reuse=reuse):
            with tf.variable_scope("action_value"):
                if feature_extraction == "cnn":
                    ...
                    for layer_size in layers:
                        #action_out = tf.layers.dense(action_out, units=layer_size, activation='relu', use_bias=False)
                        action_out = tf_layers.fully_connected(action_out, num_outputs=layer_size, activation_fn=None)
                        if layer_norm:
                            action_out = tf_layers.layer_norm(action_out, center=True, scale=True)
                        action_out = act_fun(action_out)

                #action_scores = tf.layers.dense(action_out, units=self.n_actions, activation=None, use_bias=False)
            ...
            if self.dueling:
                with tf.variable_scope("state_value"):
                    ...
                    for layer_size in layers:
                        #state_out = tf.layers.dense(state_out, units=layer_size, activation=None, use_bias=False)
                        state_out = tf_layers.fully_connected(state_out, num_outputs=layer_size, activation_fn=None)
                        if layer_norm:
                            state_out = tf_layers.layer_norm(state_out, center=True, scale=True)
                        state_out = act_fun(state_out)
                    #state_score = tf.layers.dense(state_out, units=1, activation=None, use_bias=False)
